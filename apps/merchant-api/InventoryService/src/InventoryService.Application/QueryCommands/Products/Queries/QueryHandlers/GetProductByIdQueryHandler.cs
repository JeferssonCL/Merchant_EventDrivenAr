    ...
    : IRequestHandler<GetProductByIdQuery, BaseResponse>
{
    public async Task<BaseResponse> Handle(GetProductByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        if (cachedProduct != null)
        {
            ...
            return responseHandlingHelper.Ok($"Product obtained from cache. Time: {stopwatch.ElapsedMilliseconds} ms.", cachedProduct);
        }
        ...
        logger.LogInformation("Source: Database, TimeToGet: {ElapsedTime} ms", stopwatch.ElapsedMilliseconds);
        return responseHandlingHelper.Ok($"The product has been successfully obtained from database. Time: {stopwatch.ElapsedMilliseconds} ms.", productDto);
    }
}